...
public class QsnarkAPI {
    ...
    public GetTokenReturn getAccess_Token(String client_id, String client_secret, String username, String password) throws IOException {
        ...
        String scope = "all";
        GetTokenParams getTokenParams = new GetTokenParams(client_id, client_secret, username, password, grant_type, scope);
    ...
    public InvokeConReturn invokeContract(String token, boolean _const, String from, String to, String abi, InvCallback callback, String func_name, FuncParamReal... functionParams) throws IOException, TxException, InterruptedException {
        if (token == null)
            token = "";
        boolean flag = invokeVerify(abi, func_name, functionParams);
        if (!flag)
            return new InvokeConReturn("{\"Status\":\"The method name or parameter entered does not match abi or abi is null\"}");
    ...
    public GetTxReciptReturn invokesyncContract(String token, boolean _const, String from, String to, String abi, String func_name, FuncParamReal... functionParams) throws IOException, TxException, InterruptedException {
        ...
        boolean flag = invokeVerify(abi, func_name, functionParams);
        if (!flag)
            return new GetTxReciptReturn("{\"Status\":\"The method name or parameter entered does not match abi or abi is null\"}");
    ...
    private static boolean invokeVerify(String abi, String func_name, FuncParamReal... functionParams) {
        if (abi==null||abi.equals("")){
            return false;
        }
        JSONArray jsonArray = JSONArray.fromObject(abi);
        boolean flag = false;
        for (int i = 0; i < jsonArray.size(); i++) {
            JSONObject jsonObject = (JSONObject) jsonArray.get(i);
            String temp = jsonObject.getString("inputs");
            JSONArray array = JSONArray.fromObject(temp);
            if (jsonObject.containsKey("name")) {
                String object = jsonObject.getString("name");
                if (object.equals(func_name)) {
                    if(array.size() == functionParams.length){
                        flag = true;
                        return flag;
                    }
                }
