...
import cn.qsnark.sdk.crypto.jce.*;
...
import static cn.qsnark.sdk.crypto.BIUtil.isLessThan;
import static cn.qsnark.sdk.rpc.utils.ByteUtil.bigIntegerToBytes;
...
//    public interface ECPublicKey extends PublicKey, java.security.interfaces.ECKey {
//
//        /**
...
//    }
    ...
    //TODO　从public key 恢复ECpoint 然后再恢复ECKey
    public static ECPoint extractPublicKey(final PublicKey PublicKey) {
        // 思路
        /* 首先将publicKey下转为自定义的xxPublicKey
         * 定义getX() getY()
         * x = bigInteger(pub.getX()),y = bigInteger(pub.getY())
         * new java.security.spec.ECPoint(x,y)
         * 通过 org.spongycastle.math.ec.ECPoint ecPoint = ECKey.extractPublicKey((ECPublicKey)pub);
         * ECKey ecKey = new ECKey(SpongyCastleProvider.getInstance(),pri,ecPoint);
         * System.out.println(ecKey.getPrivKeyBytes());
         */
        return null;
    }
...
//        this.publickey = pubKey;
    ...
    public static ECKey fromPrivateJSON(String accountString,String auth) throws Exception {
        JSONObject accountJSONObj = JSONObject.fromObject(accountString);
        ECKey ecKey = null;
        byte[] keyBytes = null;
        //des
        if (accountJSONObj.has("algo") && accountJSONObj.getString("algo").equals("0x02")){
            DESKeyJSON keyJson = new DESKeyJSON(accountString);
            keyBytes = DecryptKey.decryptKeyDES(keyJson, auth);
            //plain
        }else if(accountJSONObj.has("algo") && accountJSONObj.getString("algo").equals("0x03"))
        {
            PlainKey plainKey = new PlainKey(accountString);
            keyBytes = ByteUtil.hexStringToBytes(plainKey.getEncrypted());
        } else{
            keyBytes = DecryptKey.decryptKeyV3(new EncryptedKeyJSONV3(accountString, true), auth);
        }
